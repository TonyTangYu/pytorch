...
Scalar checkpoint__local_scalar_dense(Tensor const& a) {
  return at::_local_scalar_dense(decheckpoint(a));
}

Tensor checkpoint_split_with_sizes_backward(c10::ArrayRef<Tensor> a, c10::ArrayRef<long> b, long c, c10::ArrayRef<long> d, c10::TensorOptions const& e) {
...
std::vector<Tensor> checkpoint_split_with_sizes(Tensor const& a, c10::ArrayRef<long> b, long c) {
  std::vector<long> b_ = b.vec();
  rematerialize_function_t rt =
    [=](const Tensors& vec) -> Tensors {
...
Tensor checkpoint_split_backward(c10::ArrayRef<Tensor> a, long b, long c, c10::ArrayRef<long> d, const c10::TensorOptions& e) {
  std::vector<Tensor> a_ = a.vec();
  std::vector<long> d_ = d.vec();
  rematerialize_function_t rt =
    [=](const Tensors& vec) -> Tensors {
...
std::vector<Tensor> checkpoint_split(const Tensor& a, long b, long c) {
  rematerialize_function_t rt =
    [=](const Tensors& vec) -> Tensors {
...
Tensor checkpoint_expand(const Tensor& a, c10::ArrayRef<long> b, bool c) {
  std::vector<long> b_ = b.vec();
  rematerialize_function_t rt =
    [=](const Tensors& vec) -> Tensors {
...
Tensor checkpoint_diag(const Tensor& self, long diagonal) {
  rematerialize_function_t rt =
    [=](const Tensors& vec) -> Tensors {
...
Tensor& checkpoint_diag_out(Tensor& out, const Tensor& self, long diagonal) {
  mutate_function_t mt =
    [=](const Tensors& vec) {
      Tensor out_ = vec.at(0);
...
Tensor checkpoint_mv(const Tensor& self, const Tensor& vec) {
  rematerialize_function_t rt =
    [=](const Tensors& vec) -> Tensors {
...
Tensor& checkpoint_mv_out(Tensor& out, const Tensor& self, const Tensor& vec) {
  mutate_function_t mt =
    [=](const Tensors& vec) {
      Tensor out_ = vec.at(0);
...
std::tuple<Tensor, Tensor> checkpoint_nll_loss2d_forward(const Tensor& a, const Tensor& b, const Tensor& c, long d, long e) {
  rematerialize_function_t rt =
    [=](const Tensors& vec) -> Tensors {
      auto ret = at::nll_loss2d_forward(vec.at(0), vec.at(1), vec.at(2), d, e);
      return {std::get<0>(ret), std::get<1>(ret)};
    };
  auto ret = CheckpointTensorImpl::make("nll_loss2d_forward", rt, {a, b, c})[0];
  return {ret[0], ret[1]};
}

std::tuple<Tensor&, Tensor&> checkpoint_nll_loss2d_forward_out(Tensor& a, Tensor& b, const Tensor& c, const Tensor& d, const Tensor& e, long f, long g) {
  mutate_function_t mt =
    [=](const Tensors& vec) {
      Tensor a_ = vec.at(0), b_ = vec.at(1);
      at::nll_loss2d_forward_out(a_, b_, vec.at(2), vec.at(3), vec.at(4), f, g);
    };
  CheckpointTensorImpl::mutate("nll_loss2d_forward_out", mt, {a, b, c, d, e}, {0, 1});
  return {a, b};
}

Tensor checkpoint_nll_loss2d_backward(const Tensor& a, const Tensor& b, const Tensor& c, const Tensor& d, long e, long f, const Tensor& g) {
  rematerialize_function_t rt =
    [=](const Tensors& vec) -> Tensors {
      return {at::nll_loss2d_backward(vec.at(0), vec.at(1), vec.at(2), vec.at(3), e, f, vec.at(4))};
    };
  return CheckpointTensorImpl::make("nll_loss2d_backward", rt, {a, b, c, d, g})[0];
}

Tensor& checkpoint_nll_loss2d_backward_out(Tensor& a, const Tensor& b, const Tensor& c, const Tensor& d, const Tensor& e, long f, long g, const Tensor& h) {
  mutate_function_t mt =
    [=](const Tensors& vec) {
      Tensor a = vec.at(0);
      at::nll_loss2d_backward_out(a, vec.at(1), vec.at(2), vec.at(3), vec.at(4), f, g, vec.at(5));
    };
  CheckpointTensorImpl::mutate("nll_loss2d_backward_out", mt, {a, b, c, d, e, h}, {0});
  return {a};
}
