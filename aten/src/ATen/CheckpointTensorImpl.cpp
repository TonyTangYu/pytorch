...
long search_time_ = 0;
long cost_time_ = 0;
long evicted_unremated_memory_ = 0;
long eagerly_evicted_unremated_memory_ = 0;
...
Tensor checkpoint_like(const Tensor& a, const Tensor& b) {
  return is_checkpoint(b) ? checkpoint(a) : a;
}
...
void reset_profile() {
  ...
  search_time_ = 0;
  cost_time_ = 0;
  evicted_unremated_memory_ = 0;
  eagerly_evicted_unremated_memory_ = 0;
}
...
long evicted_unremated_memory() {
  return evicted_unremated_memory_;
}

long eagerly_evicted_unremated_memory() {
  return eagerly_evicted_unremated_memory_;
}
...
void AliasPool::release_resources() {
  if (is_evicted) {
    evicted_unremated_memory_ += memory;
    if (still_eagerly_evicted) {
      eagerly_evicted_unremated_memory_ += memory;
    }
  }
  tensors.clear();
  neighbors.clear();
