...
struct AliasPool : intrusive_ptr_target {
  ...
  // is the aliaspool eagerly evicted and not remated yet.
  bool still_eagerly_evicted = false;
  ...
  // TODO: why do we have is_evicted? try removing that.
  ecn_ptr ecn;
  ...
  void release_external() {
    ...
    if (external_count == 0 && evictable()) {
      if (memory > 0 && (!ecn) && (!is_evicted)) {
        evict();
        still_eagerly_evicted = true;
      }
    }
  }
